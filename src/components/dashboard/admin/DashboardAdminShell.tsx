...
export function DashboardAdminShell({ miembro }: { miembro: Miembro }) {
    ...
    return (
        <div className="flex flex-col gap-8 lg:flex-row lg:items-start lg:gap-10">
            <nav className="shrink-0 lg:w-[220px] xl:w-[240px]" aria-label="Navegación admin">
                        ...
                        <div key={gi}>
                            ...
                            <ul className="flex flex-col gap-0.5">
                                {group.items.map((item) => (
                                    <li key={item.id}>
                                        <DesktopNavBtn item={item} active={section === item.id} onClick={() => handleNav(item.id)} />
                                    </li>
                                ))}
                            </ul>
                        </div>
...
function DesktopNavBtn({ item, active, onClick }: { item: NavItem; active: boolean; onClick: () => void }) {
    return (
        <button
            type="button"
            onClick={onClick}
            ...
            className={`flex w-full items-center gap-2.5 rounded-xl px-3 py-2.5 text-left text-sm font-semibold transition-colors ${
                active ? "bg-[#0066CC] text-white shadow-sm" : "text-[#555555] hover:bg-[#F5F3EE] hover:text-[#0066CC]"
            }`}
        >
    ...
    return (
        <button
            ...
            className={`flex shrink-0 items-center gap-2 rounded-xl px-3 py-2 text-sm font-semibold transition-colors ${
                active ? "bg-[#0066CC] text-white shadow-sm" : "bg-white text-[#666666] ring-1 ring-[#E5E5E5] hover:bg-[#F5F3EE]"
            }`}
        >
            <span className={active ? "text-white" : "text-[#0066CC]"}>{item.icon}</span>
            {item.label}
