...
function EditarUsuarioModal({ miembro, empresaNombre, onClose, onSaved }: EditarUsuarioModalProps) {
    ...
    const [rol, setRol]           = useState<MiembroRol>(miembro.rol);
    ...
    async function handleGuardar() {
        ...
        try {
            ...
            if (!esAdmin && rol !== miembro.rol) {
                await gestionarUsuarioAdmin("cambiar_rol", miembro.$id, { rol });
            }
        ...
        >
            ...
            >
                ...
                <div className="flex items-center justify-between border-b border-[#E5E5E5] px-6 py-4">
                    <div>
                        <h3 className="text-base font-bold text-[#333333]">Editar usuario</h3>
                        <p className="text-xs text-[#999999]">{empresaNombre}</p>
                    </div>
                    <button
                        type="button"
                        onClick={onClose}
                ...
                <div className="space-y-4 px-6 py-5">
                    ...
                    {esAdmin ? (
                        <div className="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-xs text-red-700">
                            Este usuario es administrador global. El rol no puede modificarse desde aquí.
                        </div>
                    ) : (
                        <div>
                            <label className="mb-1 block text-xs font-semibold text-[#555555]">Rol</label>
                            <select
                                value={rol}
                                onChange={(e) => setRol(e.target.value as MiembroRol)}
                                className="w-full rounded-xl border border-[#E5E5E5] px-3 py-2 text-sm text-[#333333] outline-none focus:border-[#0066CC] focus:ring-2 focus:ring-[#0066CC]/20"
                            >
                                <option value="miembro">Miembro</option>
                                <option value="empresa_admin">Administrador de empresa</option>
                            </select>
                        </div>
                    )}
                </div>
...
export function AdminUsuarios() {
    ...
    async function cargar() {
        try {
            const [m, e] = await Promise.all([getMiembrosAdmin(), getTodasLasEmpresas()]);
            setMiembros(m);
            setEmpresas(e);
        } catch {
    ...
    const empresaMap = useMemo(
        () => new Map(empresas.map((e) => [e.$id, e.nombre_empresa])),
        [empresas],
    );
    ...
    const filtered = useMemo(() => {
        ...
        return miembros.filter((m) => {
            if (filterEmpresa && m.empresa_id !== filterEmpresa) return false;
            if (q && !m.nombre_completo.toLowerCase().includes(q) && !m.telefono.includes(q)) return false;
            return true;
        });
    }, [miembros, filterEmpresa, filterBusqueda]);
        ...
        <section className="flex flex-col gap-6">
            ...
            <header className="flex flex-col gap-4 rounded-2xl border border-[#0066CC]/15 bg-linear-to-br from-white to-[#0066CC]/6 px-5 py-6 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    ...
                    <p className="mt-1 text-sm text-[#666666]">
                        {filtered.length} de {miembros.length} usuario{miembros.length !== 1 ? "s" : ""}
                    </p>
                </div>
            ...
            <div className="flex flex-col gap-3 sm:flex-row">
                ...
                <select
                    ...
                    className="rounded-xl border border-[#E5E5E5] px-3 py-2.5 text-sm text-[#333333] outline-none focus:border-[#0066CC] focus:ring-2 focus:ring-[#0066CC]/20 sm:w-56"
                >
                            ...
                            paginaActual.map((m) => (
                                <tr key={m.$id} className="transition-colors hover:bg-[#F5F3EE]/40">
                                    ...
                                    <td className="px-4 py-3 text-[#666666]">
                                        {empresaMap.get(m.empresa_id) ?? m.empresa_id}
                                    </td>
            ...
            {editTarget && (
                <EditarUsuarioModal
                    miembro={editTarget}
                    empresaNombre={empresaMap.get(editTarget.empresa_id) ?? editTarget.empresa_id}
