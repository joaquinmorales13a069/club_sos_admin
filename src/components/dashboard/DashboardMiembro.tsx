...
const MOCK_DOCUMENTOS = [
  { id: "1", titulo: "Resultados de laboratorio", fecha: "2 abr. 2026", tipo: "PDF" },
  { id: "2", titulo: "Receta médica", fecha: "28 mar. 2026", tipo: "PDF" },
  { id: "3", titulo: "Informe de consulta", fecha: "15 mar. 2026", tipo: "PDF" },
];

const MOCK_BENEFICIOS = [
  { id: "1", titulo: "Descuento farmacia", vigencia: "Hasta jun 2026", detalle: "15% en medicamentos" },
  { id: "2", titulo: "Chequeo anual", vigencia: "1 uso por año", detalle: "Incluido en tu plan" },
];
...
function SectionPlaceholder({ title, description }: { title: string; description: string }) {
  return (
    <section className="rounded-2xl border border-[#E5E5E5] bg-white p-8 shadow-[0_1px_2px_0_rgba(0,0,0,0.04)]">
      ...
      <p className="mt-3 max-w-xl text-sm leading-relaxed text-[#666666]">{description}</p>
...
function DashboardInicio({ miembro }: { miembro: Miembro }) {
    ...
    <div className="space-y-6">
      <header className="rounded-2xl border border-[#0066CC]/15 bg-linear-to-br from-white to-[#0066CC]/6 px-5 py-6 md:px-7">
        ...
        <p className="mt-2 max-w-2xl text-sm leading-relaxed text-[#666666]">
          Aquí ves un vistazo de tu próxima cita, documentos recientes y beneficios activos. Los datos
          de ejemplo se reemplazarán cuando estén conectados al sistema.
        </p>
      </header>

      <div className="grid gap-4 lg:grid-cols-2">
        <HomeCard title="Próxima cita" icon={<IoCalendarOutline size={22} />} className="lg:col-span-2">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div className="space-y-1">
              <p className="text-lg font-semibold text-[#333333]">{MOCK_PROXIMA_CITA.especialidad}</p>
            ...
            <button
              type="button"
              className="inline-flex items-center justify-center gap-1 self-start rounded-xl bg-[#0066CC] px-4 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-[#0055aa]"
            >
              Ver detalle
            ...
            {MOCK_DOCUMENTOS.map((doc) => (
              ...
              >
                <div className="min-w-0">
                  <p className="truncate text-sm font-medium text-[#333333]">{doc.titulo}</p>
                  <p className="text-xs text-[#666666]">{doc.fecha}</p>
                </div>
        ...
        <HomeCard title="Beneficios activos" icon={<IoGiftOutline size={22} />}>
          <ul className="space-y-4">
            {MOCK_BENEFICIOS.map((b) => (
              <li key={b.id} className="rounded-xl bg-[#F5F3EE]/80 px-3 py-3">
                <p className="font-semibold text-[#333333]">{b.titulo}</p>
                <p className="mt-0.5 text-sm text-[#666666]">{b.detalle}</p>
                <p className="mt-1 text-xs text-[#666666]/90">{b.vigencia}</p>
              </li>
            ))}
          </ul>
          <button
        ...
        <HomeCard title="Tu membresía" icon={<IoMedkitOutline size={22} />}>
          <dl className="space-y-2 text-sm">
            <div className="flex justify-between gap-4">
              <dt className="text-[#666666]">Estado</dt>
              <dd className="font-semibold text-emerald-700">Activa</dd>
            </div>
            <div className="flex justify-between gap-4">
              <dt className="text-[#666666]">Parentesco</dt>
              <dd className="text-right font-medium text-[#333333]">{miembro.parentesco}</dd>
            </div>
            <div className="flex justify-between gap-4">
              <dt className="text-[#666666]">Contacto</dt>
              <dd className="text-right font-medium text-[#333333]">{miembro.telefono}</dd>
            </div>
          </dl>
        </HomeCard>
        ...
        <HomeCard title="Accesos rápidos" icon={<IoPersonOutline size={22} />} className="lg:col-span-2">
  ...
  return (
    <div className="flex flex-col gap-8 lg:flex-row lg:items-start lg:gap-10">
      ...
      <div className="min-w-0 flex-1">
        ...
        {!isSectionLoading && section === "inicio" && <DashboardInicio miembro={miembro} />}
        ...
        {!isSectionLoading && section === "beneficios" && (
          <SectionPlaceholder
            title="Beneficios"
            description="Listado completo de descuentos, programas y ventajas activas para tu membresía."
          />
        )}
